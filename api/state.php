...
try {
    ...
    if ($method==='POST') {
        ...
        $auditSummary=isset($transition)?$transition['summary']:($action==='fileLeaveApplication'?'Register leave application':($action==='updateLeaveApplication'?'Update leave application':ucfirst(strtolower(preg_replace('/(?<!^)[A-Z]/',' $0',$action)))));
