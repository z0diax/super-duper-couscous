...
test('leave registration switches dynamic fields without retaining stale values', async ({ page }) => {
  ...
  await page.reload(); const persistedRow=page.getByText('BROWSER-LEAVE-001', { exact: true }).locator('xpath=ancestor::tr'); const computeIcon=persistedRow.getByRole('button',{name:'Complete Computation'}); await expect(computeIcon).toHaveText(''); await computeIcon.click(); await expect(page.getByRole('dialog',{name:'Complete Computation'})).toBeVisible(); await expect(page.getByRole('dialog',{name:'Browser Test Applicant'})).toHaveCount(0); await page.getByRole('dialog',{name:'Complete Computation'}).getByRole('button',{name:'Cancel'}).click(); await persistedRow.getByRole('button', { name: 'View Leave Record' }).click();
