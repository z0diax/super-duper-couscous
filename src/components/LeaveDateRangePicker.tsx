...
export const LeaveDateRangePicker: React.FC<Props> = ({ startDate, endDate, dayType, onChange }) => {
  ...
  const chooseDate = (value: string) => {
    if (dayType !== 'WHOLE_DAY') { onChange(value, value); setPendingStart(null); setIsOpen(false); return; }
    if (!pendingStart) { setPendingStart(value); onChange(value, value); return; }
    onChange(pendingStart < value ? pendingStart : value, pendingStart < value ? value : pendingStart);
    setPendingStart(null); setIsOpen(false);
  };
    ...
    {isOpen && <div className="absolute left-0 z-30 mt-2 w-[min(21rem,calc(100vw-4rem))] rounded-xl border border-slate-200 bg-white p-3 shadow-2xl">
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        ...
        {days.map(date => {
          ...
          const selected = !!startDate && value >= startDate && value <= (endDate || startDate);
          const endpoint = value === startDate || value === endDate;
          return <button key={value} type="button" aria-label={value} onClick={() => chooseDate(value)} className={`h-9 rounded-lg text-xs font-semibold transition ${endpoint ? 'bg-blue-600 text-white shadow-sm' : selected ? 'bg-blue-100 text-blue-800' : inMonth ? 'text-slate-700 hover:bg-slate-100' : 'text-slate-300 hover:bg-slate-50'}`}>{date.getUTCDate()}</button>;
        })}
      </div>
