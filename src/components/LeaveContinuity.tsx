...
import { Ban, CalendarClock, CircleCheck, CirclePause, ClipboardCheck, Eye, FileCheck2, Filter, Pencil, Play, Plus, RotateCcw, Search, Send, Trash2, X } from 'lucide-react';
...
type WorkflowAction = 'compute'|'signature'|'release'|'hold'|'compliance'|'resume'|'cancel';
...
export const LeaveContinuity: React.FC = () => {
  ...
  const [workflowRecord, setWorkflowRecord] = useState<LeaveApplicationRecord | null>(null);
  ...
  const runWorkflowAction = async (event: React.FormEvent) => {
    event.preventDefault(); if (!workflowRecord || !workflowAction) return;
    const calls = {
      compute: () => completeLeaveComputation(workflowRecord.id,{remarks:actionRemarks}), signature: () => sendLeaveForSignature(workflowRecord.id,{remarks:actionRemarks}),
      release: () => releaseLeaveApplication(workflowRecord.id,{remarks:actionRemarks}), hold: () => placeLeaveOnHold(workflowRecord.id,{reason:actionRemarks}),
      compliance: () => recordLeaveCompliance(workflowRecord.id,{remarks:actionRemarks}), resume: () => resumeLeaveProcessing(workflowRecord.id,{}),
      cancel: () => cancelLeaveApplication(workflowRecord.id,{reason:actionRemarks}),
    };
    const saved = await calls[workflowAction]() as LeaveApplicationRecord | null;
    if (saved) { if (selectedRecord?.id === saved.id) setSelectedRecord(saved); setWorkflowRecord(null); setWorkflowAction(null); setActionRemarks(''); await loadRegistry(); }
  };
  const openWorkflowAction = (record: LeaveApplicationRecord, action: WorkflowAction) => { setWorkflowRecord(record); setWorkflowAction(action); setActionRemarks(''); };
  ...
  return <div className="space-y-4 pb-12">
        ...
        <table className="w-full min-w-[1200px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map(record => <tr key={record.id} className="hover:bg-slate-50/70">
              ...
              <td className="px-5 py-3 text-right"><div className="inline-flex items-center gap-1.5"><button type="button" aria-label="View Leave Record" title="View Leave Record" onClick={() => setSelectedRecord(record)} className="rounded-lg border border-blue-200 bg-white p-2 text-blue-700 shadow-xs hover:bg-blue-50"><Eye className="h-4 w-4" /></button>{canRegister && record.status === 'For_Computation' && !!record.dateRanges?.length && <button type="button" aria-label="Edit Leave Application" title="Edit Leave Application" onClick={() => openEdit(record)} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-500 hover:border-blue-200 hover:text-blue-700"><Pencil className="h-4 w-4" /></button>}{record.status==='For_Computation'&&(can('canProcess')||can('canSupervise'))&&<button type="button" aria-label="Complete Computation" title="Complete Computation" onClick={()=>openWorkflowAction(record,'compute')} className="rounded-lg border border-blue-200 bg-blue-50 p-2 text-blue-700 hover:bg-blue-100"><CircleCheck className="h-4 w-4" /></button>}{record.status==='For_Processing'&&(can('canProcess')||can('canSupervise'))&&<button type="button" aria-label="Send for Signature" title="Send for Signature" onClick={()=>openWorkflowAction(record,'signature')} className="rounded-lg border border-violet-200 bg-violet-50 p-2 text-violet-700 hover:bg-violet-100"><Send className="h-4 w-4" /></button>}{record.status==='For_Signature'&&(can('canRelease')||can('canSupervise'))&&<button type="button" aria-label="Release Leave Application" title="Release Leave Application" onClick={()=>openWorkflowAction(record,'release')} className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-emerald-700 hover:bg-emerald-100"><Send className="h-4 w-4" /></button>}{['For_Computation','For_Processing','For_Signature'].includes(record.status)&&((['For_Computation','For_Processing'].includes(record.status)&&can('canProcess'))||(record.status==='For_Signature'&&can('canApprove'))||can('canSupervise'))&&<button type="button" aria-label="Place On Hold" title="Place On Hold" onClick={()=>openWorkflowAction(record,'hold')} className="rounded-lg border border-orange-200 bg-orange-50 p-2 text-orange-700 hover:bg-orange-100"><CirclePause className="h-4 w-4" /></button>}{record.status==='On_Hold'&&((['For_Computation','For_Processing'].includes(record.heldFromStatus||'')&&can('canProcess'))||(record.heldFromStatus==='For_Signature'&&can('canApprove'))||can('canSupervise'))&&(!record.complianceReceivedAt?<button type="button" aria-label="Record Compliance Received" title="Record Compliance Received" onClick={()=>openWorkflowAction(record,'compliance')} className="rounded-lg border border-orange-200 bg-orange-50 p-2 text-orange-700 hover:bg-orange-100"><ClipboardCheck className="h-4 w-4" /></button>:<button type="button" aria-label="Resume Processing" title="Resume Processing" onClick={()=>openWorkflowAction(record,'resume')} className="rounded-lg border border-blue-200 bg-blue-50 p-2 text-blue-700 hover:bg-blue-100"><Play className="h-4 w-4" /></button>)}{can('canSupervise')&&!['Released','Approved','Disapproved','Cancelled','Pending'].includes(record.status)&&<button type="button" aria-label="Cancel Leave Record" title="Cancel Leave Record" onClick={()=>openWorkflowAction(record,'cancel')} className="rounded-lg border border-rose-200 bg-rose-50 p-2 text-rose-700 hover:bg-rose-100"><Ban className="h-4 w-4" /></button>}</div></td>
            </tr>)}
    ...
    {workflowRecord&&workflowAction&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="leave-action-title"><form onSubmit={runWorkflowAction} className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><header className="bg-slate-900 px-5 py-4 text-white"><h3 id="leave-action-title" className="font-bold">{{compute:'Complete Computation',signature:'Send for Signature',release:'Release Leave Application',hold:'Place On Hold',compliance:'Record Compliance Received',resume:'Resume Processing',cancel:'Cancel Leave Application'}[workflowAction]}</h3><p className="mt-1 text-xs text-slate-300">{workflowRecord.employeeName} · {workflowRecord.barcode || workflowRecord.trackingNumber || 'N/A'}</p></header><div className="p-5 text-xs">{workflowAction!=='resume'&&<label className="block font-semibold text-slate-700">{workflowAction==='hold'||workflowAction==='cancel'?'Reason *':workflowAction==='compliance'?'Compliance Remarks *':'Remarks (optional)'}<textarea autoFocus required={['hold','compliance','cancel'].includes(workflowAction)} value={actionRemarks} onChange={event=>setActionRemarks(event.target.value)} rows={4} className="mt-1.5 w-full rounded-lg border border-slate-300 p-2.5 font-normal outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"/></label>}{workflowAction==='resume'&&<p className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-blue-800">Resume this Leave Application at its previous processing stage?</p>}<div className="mt-5 flex justify-end gap-2"><button type="button" onClick={()=>{setWorkflowRecord(null);setWorkflowAction(null);setActionRemarks('');}} className="rounded-lg px-3 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button type="submit" className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700">Confirm Action</button></div></div></form></div>}
  </div>;
};
