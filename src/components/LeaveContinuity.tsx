...
const savedStatusRemarks = (record: LeaveApplicationRecord) => record.statusRemarks
  || (record.status === 'On_Hold' ? record.holdRemarks || record.holdReason : '')
  || (record.status === 'Released' ? record.releaseRemarks : '')
  || '';
...
export const LeaveContinuity: React.FC = () => {
  const { auditLogs, ewpRecords, currentUser, fileLeaveApplication, registerEwpRecord, updateEwpRecord, deleteEwpRecord: deleteEwp, updateLeaveApplication, deleteLeaveApplication, changeLeaveApplicationStatus, can } = useApp();
  ...
  const [editingEwpRecord, setEditingEwpRecord] = useState<EwpRecord | null>(null);
  ...
  const [remarksRecord, setRemarksRecord] = useState<LeaveApplicationRecord | null>(null);
  ...
  const [ewpDeleteRecord, setEwpDeleteRecord] = useState<EwpRecord | null>(null);
  ...
  const canModifyEwpRecord = (record: EwpRecord) => canRegister && (record.createdByUserId===currentUser.id || can('canSupervise') || can('canAdmin'));
  ...
  const resetForm = () => {
    ...
    setRemarks(''); setEwpAmount(''); setEwpPurpose(''); setIntakeMode('leave'); setEditingRecord(null); setEditingEwpRecord(null);
  };
  ...
  const confirmEwpDelete = async () => {
    ...
    const deleted = await deleteEwp(ewpDeleteRecord.id);
    if (deleted) { setEwpDeleteRecord(null); }
  };
  ...
  return <div className="space-y-4 pb-12">
    ...
    {registryMode==='leave'&&<><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs">
      <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-bold text-slate-900">Leave Registry</h2><p className="mt-0.5 text-[11px] text-slate-500">{pagination.totalRecords} matching {pagination.totalRecords===1?'record':'records'}</p></div><div className="flex items-center gap-2"><div className="relative w-full sm:w-80"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input id="leave-search-input" aria-label="Search leave applications" value={searchQuery} onChange={event=>{setSearchQuery(event.target.value);setPage(1);}} placeholder="Search employee, barcode, office or leave type" className="w-full rounded-lg border border-slate-200 py-2 pl-9 pr-3 text-xs outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"/></div>{hasActiveFilters&&<button type="button" aria-label="Clear leave search" onClick={()=>{setSearchQuery('');setFilterType('all');setFilterStatus('all');setFilterOffice('');setFiledFrom('');setFiledTo('');setLeaveDate('');setSort('registered_desc');setPage(1);}} className="shrink-0 rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" title="Clear search"><RotateCcw className="h-4 w-4"/></button>}</div></div>
        ...
        <table className="w-full min-w-[1100px] text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map(record => <tr key={record.id} className="hover:bg-slate-50/70">
              ...
              <td className="px-4 py-3"><p className="font-semibold text-slate-900">{record.employeeName}</p>{record.position&&record.position!=='Not recorded'&&<p className="text-[11px] text-slate-500">{record.position}</p>}</td>
              ...
              <td className="px-4 py-3 text-xs font-medium text-slate-700">{selectedDates(record).length > 1 ? <button type="button" onClick={() => setDatesRecord(record)} className="inline-flex whitespace-nowrap rounded-lg border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide text-blue-700 transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-100 hover:shadow-sm active:translate-y-0 active:scale-[0.98]">Multiple Dates Selected</button> : compactDates(record)}</td><td className="px-4 py-3 text-center font-bold text-slate-800">{record.totalLeaveDays ?? record.workingDaysNumber}</td>
              <td className="whitespace-nowrap px-4 py-3"><button type="button" onClick={()=>setRemarksRecord(record)} aria-label={`View status remarks for ${record.employeeName}`} title="View Status Remarks" className={`inline-flex whitespace-nowrap rounded-full border px-2.5 py-1 text-[10px] font-bold leading-none transition hover:ring-2 hover:ring-blue-100 ${statusStyle(record.status)}`}>{statusLabel(record.status)}</button></td>
    ...
    {registryMode==='ewp'&&<section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Barcode</th><th className="px-4 py-3">Name</th><th className="px-4 py-3">Office</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Purpose</th><th className="px-4 py-3">Remarks</th><th className="px-5 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredEwpRecords.map(record=><tr key={record.id} className="hover:bg-slate-50/70"><td className="px-5 py-3 font-mono font-semibold text-blue-700">{record.barcode}</td><td className="px-4 py-3 font-semibold text-slate-900">{record.employeeName}</td><td className="px-4 py-3 text-slate-600">{record.office}</td><td className="px-4 py-3 font-mono font-semibold text-slate-800">{new Intl.NumberFormat('en-PH',{style:'currency',currency:'PHP'}).format(record.amount)}</td><td className="max-w-[260px] px-4 py-3 text-slate-700">{record.purpose}</td><td className="max-w-[220px] px-4 py-3 text-slate-500">{record.remarks||'—'}</td><td className="px-5 py-3 text-right"><div className="inline-flex items-center gap-1.5">{canModifyEwpRecord(record)&&<button type="button" aria-label="Edit EWP Record" title="Edit EWP Record" onClick={()=>openEwpEdit(record)} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600 hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700"><Pencil className="h-4 w-4"/></button>}{canModifyEwpRecord(record)&&<button type="button" aria-label="Delete EWP Record" title="Delete EWP Record" onClick={()=>setEwpDeleteRecord(record)} className="rounded-lg border border-rose-200 bg-rose-50 p-2 text-rose-700 hover:bg-rose-100"><Trash2 className="h-4 w-4"/></button>}</div></td></tr>)}{filteredEwpRecords.length===0&&<tr><td colSpan={7} className="px-6 py-12 text-center"><HeartHandshake className="mx-auto h-8 w-8 text-slate-300"/><p className="mt-2 font-semibold text-slate-700">No EWP records found</p><p className="mt-1 text-xs text-slate-400">{ewpSearch?'No records match the current search.':'Registered EWP records will appear here.'}</p></td></tr>}</tbody></table></div>
    </section>}

    {isModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4 backdrop-blur-xs">
      <div className="max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <header className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-slate-900 px-5 py-4 text-white">
          <div className="flex items-center gap-3"><span className="rounded-lg bg-blue-600 p-2">{intakeMode==='ewp'?<HeartHandshake className="h-5 w-5"/>:<CalendarClock className="h-5 w-5" />}</span><div><h2 className="font-bold">{editingRecord ? 'Edit Leave Application' : editingEwpRecord ? 'Edit EWP Record' : intakeMode==='ewp' ? 'Register EWP Record' : 'Register Leave Application'}</h2><p className="text-xs text-slate-300">{editingRecord ? 'Update the official record and recalculate its Leave Days.' : editingEwpRecord ? 'Update the EWP record details below.' : intakeMode==='ewp' ? 'Record employee welfare program assistance and purpose.' : 'Register an official employee application for HRMDO processing.'}</p></div></div>
    ...
    {remarksRecord&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="leave-status-remarks-title">
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <header className="flex items-start justify-between bg-slate-900 px-5 py-4 text-white"><div><h3 id="leave-status-remarks-title" className="font-bold">Leave Status Remarks</h3><p className="mt-1 text-xs text-slate-300">{remarksRecord.employeeName} · {remarksRecord.barcode || remarksRecord.trackingNumber || 'N/A'}</p></div><button type="button" aria-label="Close Status Remarks" onClick={()=>setRemarksRecord(null)} className="rounded-lg p-1.5 text-slate-300 hover:bg-slate-800 hover:text-white"><X className="h-4 w-4"/></button></header>
        <div className="space-y-4 p-5 text-xs">
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2"><span className="text-slate-500">Current status</span><strong className="ml-2 text-slate-800">{statusLabel(remarksRecord.status)}</strong></div>
          <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-3"><p className="font-semibold text-blue-800">Remarks</p><p className="mt-1 whitespace-pre-wrap leading-5 text-slate-700">{savedStatusRemarks(remarksRecord) || 'No remarks recorded.'}</p></div>
        </div>
        <footer className="flex justify-end border-t border-slate-100 px-5 py-3"><button type="button" onClick={()=>setRemarksRecord(null)} className="rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold text-white hover:bg-blue-700">Close</button></footer>
      </div>
    </div>}
    {statusRecord&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="leave-status-title">
      <form onSubmit={submitStatusChange} className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <header className="bg-slate-900 px-5 py-4 text-white"><h3 id="leave-status-title" className="font-bold">Change Leave Status</h3><p className="mt-1 text-xs text-slate-300">{statusRecord.employeeName} · {statusRecord.barcode || statusRecord.trackingNumber || 'N/A'}</p></header>
        <div className="space-y-4 p-5 text-xs">
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2"><span className="text-slate-500">Current status</span><strong className="ml-2 text-slate-800">{statusLabel(statusRecord.status)}</strong></div>
          {savedStatusRemarks(statusRecord)&&<div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2.5"><p className="font-semibold text-blue-800">Current status remarks</p><p className="mt-1 whitespace-pre-wrap leading-5 text-slate-700">{savedStatusRemarks(statusRecord)}</p></div>}
          <label className="block font-semibold text-slate-700">New Status *<select required autoFocus value={nextStatus} onChange={event=>{setNextStatus(event.target.value as ChangeableLeaveStatus);setStatusRemarks('');}} className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white p-2.5 font-normal outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"><option value="">Select a new status</option><option value="For_Computation" disabled={statusRecord.status==='For_Computation'}>For Computation</option><option value="On_Hold" disabled={statusRecord.status==='On_Hold'}>Hold</option><option value="For_Signature" disabled={statusRecord.status==='For_Signature'}>For Signature</option><option value="Released" disabled={statusRecord.status==='Released'}>Released</option></select></label>
          {nextStatus&&nextStatus!=='For_Computation'&&<label className="block font-semibold text-slate-700">Remarks <span className="font-normal text-slate-400">(optional)</span><textarea value={statusRemarks} onChange={event=>setStatusRemarks(event.target.value)} rows={4} placeholder={`Add remarks for ${statusLabel(nextStatus)}...`} className="mt-1.5 w-full rounded-lg border border-slate-300 p-2.5 font-normal outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"/></label>}
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-4"><button type="button" onClick={()=>{setStatusRecord(null);setNextStatus('');setStatusRemarks('');}} className="rounded-lg px-3 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button type="submit" disabled={!nextStatus} className="rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50">Save Status</button></div>
        </div>
      </form>
    </div>}
    ...
    {ewpDeleteRecord&&<div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="delete-ewp-title"><div className="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><header className="bg-slate-900 px-5 py-4 text-white"><h3 id="delete-ewp-title" className="font-bold">Delete EWP Record</h3><p className="mt-1 text-xs text-slate-300">{ewpDeleteRecord.employeeName} · {ewpDeleteRecord.barcode}</p></header><div className="p-5 text-xs"><div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-900"><p className="font-bold">This EWP record will be permanently deleted.</p><p className="mt-1 leading-5 text-rose-700">Its audit event will remain as part of the system history.</p></div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={()=>setEwpDeleteRecord(null)} className="rounded-lg px-3 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancel</button><button type="button" onClick={confirmEwpDelete} className="rounded-lg bg-rose-600 px-4 py-2 font-semibold text-white hover:bg-rose-700">Delete Record</button></div></div></div></div>}
  </div>;
};
