...
export const DocumentDetailModal: React.FC = () => {
  ...
  const actionButtons = <div className="flex flex-wrap items-center gap-2">
    ...
    {(isHeld || isReadyForRecheck) && (canProcess || isAdmin) && <button type="button" onClick={() => setDialog('recheck')} className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white">{isReadyForRecheck ? 'Recheck and resume' : 'Resume processing'}</button>}
          ...
          {tab === 'workflow' && <div className="space-y-5">
            ...
            {false && !doc.isLegacyV1 && doc.status !== 'Released' && canSeeControls && <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {(isHeld || isReadyForRecheck) && (canProcess || isAdmin) && <button type="button" onClick={() => setDialog('recheck')} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white">{isReadyForRecheck ? 'Recheck and resume' : 'Resume processing'}</button>}
      ...
      {dialog && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4">
        <form onSubmit={submitDialog} className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
          ...
          <div className="mt-4 space-y-3">
            ...
            {dialog === 'recheck' && <p className="text-sm text-slate-600">{isReadyForRecheck ? 'Confirm that the submitted compliance is sufficient.' : 'Confirm that the hold requirements have been met.'} The document will resume in {phaseLabel} with the same assigned processor.</p>}
