...
export const DocumentDetailModal: React.FC = () => {
  ...
  const [releaseMode, setReleaseMode] = useState<'HRMDO Liaison' | 'External Liaison' | 'In-Person Pickup' | 'Others'>('HRMDO Liaison');
  ...
  const [nextPhaseAssigneeId, setNextPhaseAssigneeId] = useState('');
  ...
  const routesToPersonnelPool = nextPayrollStep?.assignmentSource === 'personnel_pool';
  const personnelPoolUsers = users.filter(user => nextPayrollStep?.personnelPoolUserIds?.includes(user.id));
  ...
  const execute = async () => {
    if (routesToPersonnelPool && !nextPhaseAssigneeId) return;
    ...
    await saved(() => completeStep(doc.id, remarks, actionTaken.trim() || current.requiredAction || 'Processed', [], routesToPersonnelPool ? nextPhaseAssigneeId : routesSinglePayroll ? singlePayrollProcessorId : ''));
  };
  ...
  const submitDialog = async (event: React.FormEvent) => {
    ...
    if (dialog === 'approve' && (!routesToPersonnelPool || nextPhaseAssigneeId)) await saved(() => approveDocument(doc.id, remarks || 'Approved', routesToPersonnelPool ? nextPhaseAssigneeId : ''));
    if (dialog === 'release' && releasedTo.trim() && (releaseMode !== 'Others' || otherReleaseMode.trim())) await saved(() => releaseDocument(doc.id, { releasedTo, releaseMode, otherReleaseMode: releaseMode === 'Others' ? otherReleaseMode.trim() : undefined, receiptRemarks: remarks }));
    ...
    if (dialog === 'external-return' && (!routesToPersonnelPool || externalResult === 'Disapproved' || nextPhaseAssigneeId)) await saved(() => recordExternalReturn(doc.id, { returnedFrom: current.externalHandoff?.destinationOffice || '', returnedBy, result: externalResult, remarks, files: returnFile ? [returnFile] : [], nextAssigneeId: routesToPersonnelPool ? nextPhaseAssigneeId : undefined }));
  };
  ...
  const actionButtons = <div className="flex flex-wrap items-center gap-2">
    ...
    {(isHeld || isReadyForRecheck) && canProcess && <button type="button" onClick={() => setDialog('recheck')} className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white">{isReadyForRecheck ? 'Review and resume' : 'Comply and resume'}</button>}
    {canProcess && <button type="button" onClick={() => setDialog('remark')} className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700">Add remark</button>}
    {canProcess && <button type="button" onClick={() => setDialog('upload')} className="inline-flex items-center gap-1 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700"><Upload className="h-4 w-4" />Upload file</button>}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm">
      <section className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <main className="flex-1 overflow-y-auto p-5">
          {tab === 'workflow' && <div className="space-y-5">
            {doc.status === 'Disapproved' && <div className="rounded-xl border border-rose-300 bg-rose-50 p-4 text-rose-950"><strong>Document was disapproved</strong><p className="mt-1 text-xs">The external office returned this document as disapproved, so it will not continue to the remaining phases.{current.externalReturn?.remarks ? ` Remarks: ${current.externalReturn.remarks}` : ''}</p></div>}
                ...
                const phaseMeta = completed
                  ...
                  : step.stageType === 'EXTERNAL_HANDOFF_REVIEW'
                    ...
                    : step.payrollAssignmentSource === 'employment_routing'
                      ...
                      : step.assignmentSource === 'personnel_pool'
                        ? `Personnel selected during Phase ${step.stepNumber - 1}`
            ...
            {false && !doc.isLegacyV1 && doc.status !== 'Released' && canSeeControls && <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {(isHeld || isReadyForRecheck) && canProcess && <button type="button" onClick={() => setDialog('recheck')} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white">{isReadyForRecheck ? 'Review and resume' : 'Comply and resume'}</button>}
                {canProcess && <button type="button" onClick={() => setDialog('remark')} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700">Add remark</button>}
                {canProcess && <button type="button" onClick={() => setDialog('upload')} className="inline-flex items-center gap-1 rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700"><Upload className="h-4 w-4" />Upload file</button>}
        ...
        <form onSubmit={submitDialog} className="w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
          ...
          <div className="mt-4 space-y-3">
            ...
            {dialog === 'complete' && <>
              ...
              {routesToPersonnelPool && <label className="block text-sm font-semibold text-slate-700">Assign Phase {doc.currentStepNumber + 1} to<select required value={nextPhaseAssigneeId} onChange={event => setNextPhaseAssigneeId(event.target.value)} className="mt-1 w-full rounded-lg border border-blue-300 bg-white p-2 text-sm"><option value="">Choose eligible personnel...</option>{personnelPoolUsers.map(user => <option key={user.id} value={user.id}>{user.name} — {user.roleTitle}</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">{nextPayrollStep.name}</span></label>}
            ...
            {dialog === 'approve' && <>{routesToPersonnelPool && <label className="block text-sm font-semibold text-slate-700">Assign Phase {doc.currentStepNumber + 1} to<select required autoFocus value={nextPhaseAssigneeId} onChange={event => setNextPhaseAssigneeId(event.target.value)} className="mt-1 w-full rounded-lg border border-blue-300 bg-white p-2 text-sm"><option value="">Choose eligible personnel...</option>{personnelPoolUsers.map(user => <option key={user.id} value={user.id}>{user.name} — {user.roleTitle}</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">{nextPayrollStep.name}</span></label>}<textarea autoFocus={!routesToPersonnelPool} value={remarks} onChange={event => setRemarks(event.target.value)} placeholder="Approval remarks" className="min-h-24 w-full rounded-lg border border-slate-300 p-2 text-sm" /></>}
            {dialog === 'release' && <><input autoFocus required value={releasedTo} onChange={event => setReleasedTo(event.target.value)} placeholder="Released to" className="w-full rounded-lg border border-slate-300 p-2 text-sm" /><select aria-label="Release method" value={releaseMode} onChange={event => { setReleaseMode(event.target.value as typeof releaseMode); setOtherReleaseMode(''); }} className="w-full rounded-lg border border-slate-300 bg-white p-2 text-sm">{['HRMDO Liaison','External Liaison','In-Person Pickup','Others'].map(option => <option key={option}>{option}</option>)}</select>{releaseMode === 'Others' && <input required value={otherReleaseMode} onChange={event => setOtherReleaseMode(event.target.value)} placeholder="Specify other release method" className="w-full rounded-lg border border-slate-300 p-2 text-sm" />}<textarea value={remarks} onChange={event => setRemarks(event.target.value)} placeholder="Receipt remarks (optional)" className="min-h-20 w-full rounded-lg border border-slate-300 p-2 text-sm" /></>}
            ...
            {dialog === 'external-return' && <><input autoFocus value={returnedBy} onChange={event => setReturnedBy(event.target.value)} placeholder="Returned by / representative" className="w-full rounded-lg border border-blue-300 p-2 text-sm" /><select required value={externalResult} onChange={event => setExternalResult(event.target.value)} className="w-full rounded-lg border border-blue-300 bg-white p-2 text-sm"><option value="">Select result</option>{['Approved','Approved with Comments','Returned with Comments','Signed','Reviewed','Disapproved','No Action','Other'].map(value => <option key={value}>{value}</option>)}</select>{externalResult === 'Disapproved' && <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs text-rose-800"><strong>Choosing Disapproved will end this workflow.</strong><p className="mt-1">The document will not continue to the remaining phases.</p></div>}{routesToPersonnelPool && externalResult !== 'Disapproved' && <label className="block text-sm font-semibold text-slate-700">Assign Phase {doc.currentStepNumber + 1} to<select required value={nextPhaseAssigneeId} onChange={event => setNextPhaseAssigneeId(event.target.value)} className="mt-1 w-full rounded-lg border border-blue-300 bg-white p-2 text-sm"><option value="">Choose eligible personnel...</option>{personnelPoolUsers.map(user => <option key={user.id} value={user.id}>{user.name} — {user.roleTitle}</option>)}</select></label>}<textarea value={remarks} onChange={event => setRemarks(event.target.value)} placeholder="Return remarks (optional)" className="min-h-20 w-full rounded-lg border border-blue-300 p-2 text-sm" /><label className="block rounded-lg border border-dashed border-blue-300 p-3 text-sm">Attach returned file<input type="file" className="mt-2 block w-full text-xs" onChange={event => setReturnFile(event.target.files?.[0] || null)} /></label></>}
          </div>
